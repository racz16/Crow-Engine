...
export class GlTexture2D extends GlTexture implements ITexture2D, IGlFboAttachment {
    ...
    //store
    public store(data: TexImageSource, format: GlFormat, flipYAxis = true, mipmapLevel = 0, offset: ReadonlyVec2 = vec2.create()): void {
    ...
    public storeFromBinary(data: ArrayBufferView, size: ReadonlyVec2, format: GlFormat, type: GlTextureDataType, flipYAxis = true, mipmapLevel = 0, offset: ReadonlyVec2 = vec2.create()): void {
        const glFormat = GlFormatResolver.enumToGl(format);
    ...
    public static createTextureFromBinary(data: ArrayBufferView, size: ReadonlyVec2, internalFormat: GlInternalFormat, mipmaps = true, flipY = true): GlTexture2D {
