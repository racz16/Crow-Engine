...
export class GlTexture2D extends GlTexture implements ITexture2D, IFboAttachment {
    ...
    //store-------------------------------------------------------------------------------------------------------------
    //
    public store(data: HTMLImageElement, format: Format, flipYAxis = true, offset = vec2.create()): void {
        const glFormat = FormatResolver.enumToGl(format);
        this.bind();
        Gl.gl.pixelStorei(Gl.gl.UNPACK_FLIP_Y_WEBGL, flipYAxis)
        Gl.gl.texSubImage2D(this.getTarget(), 0, offset[0], offset[1], glFormat, Gl.gl.UNSIGNED_BYTE, data);
    }
